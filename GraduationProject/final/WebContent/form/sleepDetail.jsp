...
<body onload="initTmap()">
	...
	<div id="content">
		<c:set var="s_d" value="${requestScope.sleepList }" />
		<c:set var="s_r_d" value="${requestScope.sleepRList }" />
		<c:set var="s_l_d" value="${requestScope.sleepLList }" />
		<script>
								var visible2 = false;
		...
		<table>
				...
				<td align="center">
					<table>
						<tr>
							<td>
								...
								<div id="likeMember">
									<table>
										...
										<tr>
											<c:forEach items="${s_l_d}" var="sld">
												<td align='center'><img src="files/${sld.m_img }"
													class="like_pic"> <br>
													<p class='like_font'>${sld.s_l_m_id }</p></td>
											</c:forEach>
										</tr>
									</table>
								</div>
							...
							<td style="width: 100px;"></td>
							<td><p class='section_font'>댓글 수 : ${s_d.s_r_count }</p></td>
						</tr>
					</table>
				</td>
				<td style="width: 100px;"></td>
			...
			<tr>
				<td>
					<div class="info_div">
						<table>
						<c:forEach items="${s_r_d}" var="srd">
							<tr>
							<td>						
							<img src="files/${srd.m_img}" class="member_pic">
							</td>
							<td>
							${srd.s_r_m_id }
							${srd.s_r_content }
					...
					<div class="info_div">
						...
						<table>
							...
							<tr>
								...
								<a onclick="memberDetail()"><img src="img/idicon3.png"/></a>&nbsp;${s_d.s_m_id }
								<div id="submenu">
									<div>
										<a href="memberDetail.do?m_id=${s_d.s_m_id }">회원정보</a>
									</div>
									<div>
										<a href="meet_friend.do?f_id=${s_d.s_m_id }">친구맺기</a>
									</div>
								</div>
						...
						<table align="center">
							...
							<tr>
								<td align="center">${s_d.s_price}만원대</td>
								<td align="center">${s_d.s_session}</td>
								<td align="center">${s_d.s_inout}</td>
							</tr>
						</table>
					</div>
				</td>
			...
			<tr>
				<td align="center">
				<form id='like_btn' method='post' action='enterSleepReply.do'>
					<input type="hidden" name="id" value="${s_d.s_m_id}">
						...
						<script>
							function checkLike() {
								...
								if (isOk == true) {
									like_Btn.action = "enterSleepLike.do"
									like_Btn.submit();
								} else {
									//Nothing
								}
						...
						<input type="textarea" name='s_content_Rply' style="width: 330px; text-align: center"><input
							type='submit'>
					</form>
				</td>
				<td style="width: 100px;"></td>
		...
		<div class="content_div" align="center">${s_d.s_content}</div>
